...
void Decrypt_Join(uint8_t *key, uint8_t *buffer,  uint8_t *decBuffer, uint16_t size)
{
	    aes_context AesContext;
			mem_Init( AesContext.ksch, '\0', 240 );  //Decrypt
			aes_set_key( key, 16, &AesContext );
			aes_encrypt( buffer, decBuffer, &AesContext );	
			if( size >= 16 )  // Check if optional CFList is included
			{
...
{
		uint8_t Aes_Cmac_LastBlockSize;
		uint8_t Aes_Cmac_LastBlock[16];
		uint8_t Aes_Cmac_In[16];
		aes_context    rijndael;	
		uint8_t K[16];
    uint8_t digest[AES_CMAC_DIGEST_LENGTH];
		mem_Init(Aes_Cmac_In, 0, sizeof Aes_Cmac_In);
		mem_Init(rijndael.ksch, '\0', 240);
		Aes_Cmac_LastBlockSize = 0;
	  aes_set_key(key, AES_CMAC_KEY_LENGTH, &rijndael);

		while (len > 16)  //输入数据分组，每组16B，对每一组分别进行加密
		{     
			XOR(data, Aes_Cmac_In);
		  aes_encrypt(Aes_Cmac_In, Aes_Cmac_In, &rijndael);
      data += 16;
      len -= 16;
		}          
    mem_Copy(Aes_Cmac_LastBlock, data, len);   //最后一个分组，直接存储
    Aes_Cmac_LastBlockSize = len;	             //最后一个分组的字节数(<=16B)

		mem_Init(K, '\0', 16); /* generate subkey K1 */
		aes_encrypt( K, K, &rijndael);
		if (K[0] & 0x80) 
		{
			LSHIFT(K, K);
			K[15] ^= 0x87;
			...
			LSHIFT(K, K);
    if (Aes_Cmac_LastBlockSize == 16)   //最后一个分组正好是16B，即一个完整的分组
			XOR(K, Aes_Cmac_LastBlock);			
		else                                //最后一个分组不满16B
		{			
				if (K[0] & 0x80) /* generate subkey K2 */
				{
					LSHIFT(K, K);
					K[15] ^= 0x87;
				} 
					...
					LSHIFT(K, K);
				Aes_Cmac_LastBlock[Aes_Cmac_LastBlockSize] = 0x80;	  //0x80作为数据结尾
				while (++Aes_Cmac_LastBlockSize < 16)                 //填充最后一个分组
						 Aes_Cmac_LastBlock[Aes_Cmac_LastBlockSize] = 0;  //空字节用0填充
				XOR(K, Aes_Cmac_LastBlock); 
		 }
    XOR(Aes_Cmac_LastBlock, Aes_Cmac_In);
    aes_encrypt(Aes_Cmac_In, digest, &rijndael);	 
		*mic = ( uint32_t )( digest[3] << 24 | digest[2] << 16 | digest[1] << 8 | digest[0] );
}
...
void AES_CMAC2(uint8_t key[AES_CMAC_KEY_LENGTH],uint8_t *data1,uint8_t len1,uint8_t *data2,uint8_t len2, uint32_t *mic)
{
    ...
    uint8_t digest[AES_CMAC_DIGEST_LENGTH];
	  uint8_t mlen;	
		...
		mem_Init(rijndael.ksch, '\0', 240);
		Aes_Cmac_LastBlockSize = 0;
	  aes_set_key(key, 16, &rijndael);	
							
		while (len1 > 16) 
		{      
			XOR(data1, Aes_Cmac_In);
		  aes_encrypt( Aes_Cmac_In, Aes_Cmac_In, &rijndael);
      data1 += 16;
      len1 -= 16;
		}
    mem_Copy(Aes_Cmac_LastBlock, data1, len1);
    Aes_Cmac_LastBlockSize = len1;		
	
		mlen = MIN(16-Aes_Cmac_LastBlockSize, len2); 
		mem_Copy(Aes_Cmac_LastBlock+Aes_Cmac_LastBlockSize, data2, mlen); //衔接
		Aes_Cmac_LastBlockSize += mlen;
		if (Aes_Cmac_LastBlockSize<16 || len2==mlen)
			return;
		XOR(Aes_Cmac_LastBlock, Aes_Cmac_In);
		aes_encrypt( Aes_Cmac_In, Aes_Cmac_In, &rijndael);
		data2 += mlen;
		...
		while (len2 > 16) 
		{      
			XOR(data2, Aes_Cmac_In);
		  aes_encrypt( Aes_Cmac_In, Aes_Cmac_In, &rijndael);
      data2 += 16;
      len2 -= 16;
		}
    mem_Copy(Aes_Cmac_LastBlock, data2, len2);
    Aes_Cmac_LastBlockSize = len2;	
    ...
    if (Aes_Cmac_LastBlockSize == 16) 
		...
		else 
		{
				if (K[0] & 0x80) 
				{
					...
					LSHIFT(K, K);
				Aes_Cmac_LastBlock[Aes_Cmac_LastBlockSize] = 0x80;
				while (++Aes_Cmac_LastBlockSize < 16)
						 Aes_Cmac_LastBlock[Aes_Cmac_LastBlockSize] = 0;
				XOR(K, Aes_Cmac_LastBlock); 
		}
    XOR(Aes_Cmac_LastBlock, Aes_Cmac_In);
    aes_encrypt(Aes_Cmac_In, digest, &rijndael);
